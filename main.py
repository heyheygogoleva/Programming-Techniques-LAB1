...
time_select = []
for i in for_sort:
    select_table, per  = select_sort(i)
    print(len(i), ' time ', per)
    time_select.append(per)
     ready_table = class_to_table(select_table)
     table_name = 'select_sort' + str(len(i)) + '.xlsx'
     ready_table.to_excel(table_name, index=False)
     
# сортировка простыми вставками
time_insert = []
for i in for_sort:
    insert_table, per  = insert_sort(i)
    print(len(i), ' time ', per)
    time_insert.append(per)
     ready_table = class_to_table(insert_table)
     table_name = 'insert_sort' + str(len(i)) + '.xlsx'
     ready_table.to_excel(table_name, index=False)

# пирамидальная сортировка
time_pyramid = []
for i in for_sort:
    pyramid_table, per  = pyramid_sort(i)
    print(len(i), 'time', per)
    time_pyramid.append(per)
    ready_table = class_to_table(pyramid_table)
    table_name = 'pyramid_sort' + str(len(i)) + '.xlsx'
    ready_table.to_excel(table_name, index=False)
...
plt.plot(time_select, y, label = 'выбором')
plt.plot(time_insert, y, label = 'вставками')
plt.plot(time_pyramid, y, label = 'пирамидальная')
...
# plt.plot(time_insert, y, label = 'вставками')
# plt.plot(time_pyramid, y, label = 'пирамидальная')
plt.title('Время работы сортировки выбором (сек)')
plt.ylabel("количество объектов")
plt.xlabel("время")
plt.legend()
plt.show()
...
plt.plot(time_insert, y, label = 'вставками')
# plt.plot(time_pyramid, y, label = 'пирамидальная')
plt.title('Время работы сортировки вставками (сек)')
plt.ylabel("количество объектов")
plt.xlabel("время")
plt.legend()
plt.show()
...
# plt.plot(time_insert, y, label = 'вставками')
plt.plot(time_pyramid, y, label = 'пирамидальная')
plt.title('Время работы пирамидальной сортировки (сек)')
plt.ylabel("количество объектов")
plt.xlabel("время")
plt.legend()
plt.show()
...
time_df = pd.DataFrame({'размер данных': y, 'сортировка выбором': time_select, 'сортировка простой вставкой': time_insert, 'пирамидальная сортировка': time_pyramid})
time_df
